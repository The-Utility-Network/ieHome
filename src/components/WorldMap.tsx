                    ...
                    <Marker key={loc.slug} coordinates={[loc.coordinates.x, loc.coordinates.y]}>
                        ...
                        >
                            <Link href={`/locations/${loc.slug}`}>
                                ...
                                <circle r={8} fill="transparent" stroke="#A855F7" strokeWidth={1} opacity={0.5} />

                                {hoveredLocation === loc.slug && (
                                    <foreignObject x={15} y={-40} width={200} height={100}>
                                        <div className="glass-panel p-2 rounded-lg border border-[#A855F7]/30 bg-black/90 text-xs">
                                            <div className="font-bold text-white">{loc.city}</div>
                                            <div className="text-[#A855F7]">{loc.keyFocus}</div>
                                        </div>
                                    </foreignObject>
                                )}
                            </Link>
